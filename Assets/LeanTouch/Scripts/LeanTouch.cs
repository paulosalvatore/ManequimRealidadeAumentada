...
namespace Lean
{
	...
	public class LeanTouch : MonoBehaviour
	{
		...
		public static void RotateObject(Transform transform, Vector2 deltaPosition, Camera camera = null)
		{
			if (transform != null && deltaPosition.x != 0.0f)
			{
				transform.rotation = RotateObject(transform.rotation, deltaPosition, camera);
			}
		}
		
		public static Quaternion RotateObject(Quaternion worldRotation, Vector2 deltaPosition, Camera camera = null)
		{
			if (deltaPosition.x != 0.0f)
			{
				...
				if (camera != null)
				{
					worldRotation = Quaternion.AngleAxis(deltaPosition.x * 0.2f, Vector3.up) * worldRotation;
				}
			}
